...
document.addEventListener('DOMContentLoaded', () => {
    ...
    (function initWeather() {
        ...
        const WMO = {
            0: ['Acik', '☀️'], 1: ['Az Bulutlu', '🌤️'], 2: ['Parcali Bulutlu', '⛅'], 3: ['Kapali', '☁️'],
            45: ['Sisli', '🌫️'], 48: ['Kiragili Sis', '🌫️'], 51: ['Hafif Cisenti', '🌦️'], 53: ['Cisenti', '🌦️'],
            55: ['Yogun Cisenti', '🌧️'], 61: ['Hafif Yagmur', '🌧️'], 63: ['Yagmur', '🌧️'], 65: ['Siddetli Yagmur', '🌧️'],
            71: ['Hafif Kar', '🌨️'], 73: ['Kar', '❄️'], 75: ['Yogun Kar', '❄️'],
            80: ['Saganak', '🌦️'], 81: ['Kuvvetli Saganak', '⛈️'], 82: ['Siddetli Saganak', '⛈️'],
            95: ['Firtina', '⛈️'], 96: ['Dolu', '⛈️'], 99: ['Siddetli Dolu', '⛈️']
        };
        const dirs = ['K', 'KD', 'D', 'GD', 'G', 'GB', 'B', 'KB'];
        const windDir = d => dirs[Math.round(d / 45) % 8];

        let wxData = null;
        let wxLocation = 'Konum alinamadi';
        ...
        async function fetchWx(lat, lon) {
            const p = new URLSearchParams({
                ...
                current: ['temperature_2m', 'relative_humidity_2m', 'apparent_temperature',
                    ...
                    'surface_pressure', 'visibility'].join(','),
                wind_speed_unit: 'kmh', timezone: 'auto'
            });
            const r = await fetch(`${WX_API}?${p}`);
            if (!r.ok) throw new Error('wx error');
            return r.json();
        }

        async function fetchLoc(lat, lon) {
            try {
                const r = await fetch(`${GEO_API}?lat=${lat}&lon=${lon}&format=json&accept-language=tr`);
                const d = await r.json();
                const city = d.address?.city || d.address?.town || d.address?.county || '';
                const cc = d.address?.country_code?.toUpperCase() || '';
                return city ? `${city}, ${cc}` : `${lat.toFixed(2)}°, ${lon.toFixed(2)}°`;
            } catch { return `${lat.toFixed(2)}°N ${lon.toFixed(2)}°E`; }
        }
        ...
        function renderWidget() {
            if (!wxData) {
                ...
                document.getElementById('wxError').style.display = 'block';
                document.getElementById('wxErrorText').textContent = 'Hava durumu alinamadi.';
                return;
            }
            const c = wxData.current;
            const [desc, icon] = WMO[c.weather_code] || ['Bilinmiyor', '🌡️'];
            const wind = Math.round(c.wind_speed_10m);

            document.getElementById('wxLocation').textContent = wxLocation;
            document.getElementById('wxIcon').textContent = icon;
            document.getElementById('wxTempVal').textContent = Math.round(c.temperature_2m);
            ...
            document.getElementById('wxWind').textContent = `${wind} km/s ${windDir(c.wind_direction_10m)}`;
            document.getElementById('wxVisibility').textContent = c.visibility >= 10000 ? '>10 km' : (c.visibility / 1000).toFixed(1) + ' km';
            document.getElementById('wxPressure').textContent = `${Math.round(c.surface_pressure)} hPa`;
            document.getElementById('wxUpdated').textContent = new Date().toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });

            const alertEl = document.getElementById('wxAlert');
            if (wind > 60 || [95, 96, 99, 81, 82].includes(c.weather_code)) {
                alertEl.style.display = 'block';
                document.getElementById('wxAlertText').textContent = wind > 60 ? `Kuvvetli ruzgar: ${wind} km/s` : 'Firtina uyarisi aktif';
            } else {
        ...
        async function init() {
            const lat = 36.2023; // Antakya
            const lon = 36.1613; // Antakya
            try {
                wxData = await fetchWx(lat, lon);
                wxLocation = 'Antakya, Türkiye';
            } catch {
                wxLocation = 'Veri alinamadi';
            }
            // Buton alt yazısını güncelle
            const sub = document.getElementById('weatherBtnSub');
            if (sub && wxData) {
                const [desc] = WMO[wxData.current.weather_code] || ['Bilinmiyor'];
                sub.textContent = `${Math.round(wxData.current.temperature_2m)}°C — ${desc}`;
            }
            renderWidget();
        }

        // Widget aç/kapat işlemleri
        document.getElementById('weatherBtn').addEventListener('click', () => {
            document.getElementById('wxWidget').classList.toggle('active');
        });
        document.getElementById('wxCollapseBtn').addEventListener('click', () => {
            document.getElementById('wxWidget').classList.toggle('collapsed');
        });
